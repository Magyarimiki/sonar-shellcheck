...
public class ShellCheckPlugin implements Plugin {
    ...
    public void define(Context context) {
        boolean isShellRegistered = context.getBootConfiguration().get(ShellCheckSettings.FILE_SUFFIXES_KEY).isPresent();
        if (!isShellRegistered) {
            ...
            context.addExtensions(ShellCheckSettings.getConditionalProperties());
        }
	...
	context.addExtension(PropertyDefinition.builder("sonar.shell.isBuiltin")
            .name("Shell support is built in")
            .description("Shell support is built in")
            .type(PropertyType.BOOLEAN)
            .defaultValue(String.valueOf(isShellRegistered))
            .category("ShellCheck")
            .hidden()
            .build()
	);
